...
JobEngine.Collections.Resumes = Backbone.Collection.extend({
	...
	nextPage : function(options){		
		...
		var i =0;
		// var list_exists = _.map (this.models, function (model) { return model.attributes.ID});

		// $(listView).find('li.post-approve').each(function(){
		// 	var vclass = $(this).attr('class');
		// 	var str = vclass.split(" ");			
		// 	list_exists.push(str[str.length-1]);
		// 	i++;
		// });	

		// if(list_exists.length >=1 )
		// 	collection.fetchData.post__not_in = list_exists;
		...
		if (typeof resp.data.resumes != 'undefined'){
			return _.map( resp.data.resumes, function (data){
				...
				if ( typeof resp.data.jobseekers != 'undefined' ){
					var jobseeker = false;
					//console.log (resp.data.jobseekers);
...
JobEngine.Views.ResumeListItem = Backbone.View.extend({
	...
	initialize: function(params){
		...
		//console.log ('new resume list item');
	},
...
JobEngine.Views.EducationView = Backbone.View.extend({
	...
	initialize: function(params){
		...
		this.onDelete = params.onDelete || function(){ console.log('on delete education')};
	},
	toObject: function(){
		var element = this.$el;
