...
function je_process_job_paid ($job_data){
	...
	if( (isset($plans[$job_data['job_package']]['price']) && $plans[$job_data['job_package']]['price'] == 0) ){
		...
		*/
		...
		// $paid_plan	=	je_get_company_paid_plans ($job_data['post_author']);
		// if(isset($paid_plan[$job_data['job_package']]) && $paid_plan[$job_data['job_package']] === 0)
		// 	$job_data['job_paid'] = 0;
		// else 
		// 	$job_data['job_paid'] = 1;
...
function et_ajax_get_statistic(){
	...
	try {
		...
		// get lastest companies
		// $et_after_time = $within;
		// $query_payments = new WP_Query(array(
		// 	'post_type' 		=> 'order',
		...
		// $payments 	= array();
		// while( $query_payments->have_posts() ){
		// 	$query_payments->the_post();
		// 	if ( empty( $post->post_parent ) ) continue;

		// 	$job = get_post($post->post_parent);
		// 	$payments[] = et_create_jobs_response( $job );
		// }
...
function et_get_post_job_url( $url, $pagename = '', $params = array() ){
	...
	// if ( ($pagename == 'page-process-payment' || $pagename == 'process-payment') 
	// 	AND !empty($params['paymentType']) ){
	// 	$url = strtok($url, '?');
	// 	$url .= $params['paymentType'];
	// 	if(isset($params['job_id'])) $url .= '?job_id='.$params['job_id'];
	// }
