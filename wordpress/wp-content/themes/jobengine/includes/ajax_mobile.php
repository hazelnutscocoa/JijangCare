	...
	$response = array('success' => false, 'message' => __('Register failed ', ET_DOMAIN) );
		try {
			...
			if(!empty($error)) {
				...
				// Send email after register successfully
				// if (get_option('register_email_notification') == "Enable" || get_option('register_email_notification') == "") {
...
function et_ajax_mobile_main_query(){
	...
	try {
		...
		//print_r($query_job_load_more->max_num_pages);
		//print_r($query_job_load_more);exit();
