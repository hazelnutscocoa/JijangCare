...
class ET_Widget_Job_Categories extends WP_Widget 
{
	...
	function update ( $new_instance, $old_instance ) {
		$instance = $old_instance;
		$new_instance = wp_parse_args((array) $new_instance, array( 'title' => __('JOB CATEGORIES', ET_DOMAIN )));
		$instance['title'] = strip_tags($new_instance['title']);
		$instance['hide_empty'] = strip_tags($new_instance['hide_empty']);
...
function et_widgets_init () {
	...
	/*if($check < 2) {
		...
		$companies_count	=	get_option('widget_je_company_count', array('_multiwidget' => 1));
		$profile			=	get_option('widget_je_company_profile', array('_multiwidget' => 1));

	    $next_companies_count	=	et_next_widget_number('je_company_count', 'widget_je_company_count') ;
		$next_profile			=	et_next_widget_number('je_company_profile', 'widget_je_company_profile' ) ;
		$next_profile_2			=	et_next_widget_number('je_company_profile', 'widget_je_company_profile') ;

		$companies_count[$next_companies_count]	=	array ('title' => '');
	    $profile[$next_profile]					=	array ('title' => '');
	    $profile[$next_profile_2]				=	array ('title' => '');

		update_option('widget_je_company_count', $companies_count );
		update_option('widget_je_company_profile', $profile );

		if(isset($sidebars_widgets['sidebar-companies'])) {
			array_unshift ($sidebars_widgets['sidebar-companies'] , 'je_company_count-'.$next_companies_count );
		}

		if(isset($sidebars_widgets['sidebar-company'])) {
			array_unshift ($sidebars_widgets['sidebar-company'] , 'je_company_profile-'.$next_profile );
		}

		if(isset($sidebars_widgets['sidebar-job-detail'])) {
			array_unshift ($sidebars_widgets['sidebar-job-detail'] , 'je_company_profile-'.$next_profile_2 );
		}
		
		wp_set_sidebars_widgets($sidebars_widgets);
		update_option('et_first_install_widget', 2 );
